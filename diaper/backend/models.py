...
class AttractorPerceiver(torch.nn.Module):
    ...
    def forward(
        ...
        for i in range(self.n_layers):
            ...
            if self.condition_frame_encoder:
                if args.length_normalize:
                    ...
                    logits = torch.matmul(e.clone(), atts.permute(0, 2, 1)) # (B, T, n_attractors)
                if self.context_activations:
                ...
                # (B, T, n_attractors) x (B, n_attractors, d_latents) x (d_latents, d_latents)
                # condition: (B, T, d_latents)
        ...
        # everything is stacked with last dimension is number of blocks
    ...
    def get_attractors(
        ...
        '''
            - per_prcvblock_latents: `(n_blocks, B, n_latents, d_latents)`
            - per_prcvblock_attractors: `[(B, n_attractors, d_latents)]`
            - per_prcvblock_logits: `[(B, n_attractors)]`
            - per_prcvblock_l2a_entropy_term: `[(1,)]`
        '''
        # frame_embs: (B, T, E)
        per_prcvblock_attractors = []
        per_prcvblock_logits = []
        per_prcvblock_l2a_entropy_term = []
        latents = self.latent_attractors.repeat((frame_embs.shape[0], 1, 1)) # (B, n_latents, d_latents)
        if self.use_pre_crossattention: # cross-attention
            # latents: (B, n_latents, d_latents)
            latents = self.pre_crossattention(
                latents, frame_embs, frame_embs)[0]
        # per_prcvblock_latents: (n_block, B, n_latents, d_latents)
        ...
        for i in range(len(per_prcvblock_latents)):
            ...
            # attractors_i: (B, n_attractors, d_latents) -> (B, n_attractors, 1) -> (B, n_attractors)
