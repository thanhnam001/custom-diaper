...
# Runs the 3-stage SC_LibriSpeech_2spk_conformer_kernel5 recipe end-to-end
...
#
# Configs: models/10attractors/SC_LibriSpeech_2spk_conformer_kernel5/*.yaml
...
# etc.) with exactly one change: conformer_conv_kernel_size: 5 instead of 3
# (conv_norm_type stays batchnorm, same as that baseline -- see
# models/10attractors/SC_LibriSpeech_2spk_conformer_layernorm for the
# LayerNorm variant). Everything else -- including latents2attractors:
...
CONFIG_DIR="models/10attractors/SC_LibriSpeech_2spk_conformer_kernel5"
# Respects an already-set CUDA_VISIBLE_DEVICES from the calling shell (e.g.
# `CUDA_VISIBLE_DEVICES=1 ./scripts/run_pipeline_conformer_kernel5.sh`),
