...
# Runs the 3-stage SC_LibriSpeech_2spk_conformer_kernel3_6blocks recipe
# end-to-end on a single GPU (tuned for 1x V100 32GB; configs already set
# num_workers: 4):
...
#
# Configs: models/10attractors/SC_LibriSpeech_2spk_conformer_kernel3_6blocks/*.yaml
...
# etc.) with exactly one change: frame_encoder_layers: 6 instead of 4
# (conformer_conv_kernel_size stays 3, conv_norm_type stays batchnorm, same
# as that baseline). Everything else -- including latents2attractors:
# weighted_average -- matches that baseline, so the two recipes differ in
# only that one variable. Batch sizes are unchanged from the 4-block
# baseline; lower them first if a deeper frame encoder OOMs on the V100
# (see the train.yaml comment in the config dir).
#
...
CONFIG_DIR="models/10attractors/SC_LibriSpeech_2spk_conformer_kernel3_6blocks"
# Respects an already-set CUDA_VISIBLE_DEVICES from the calling shell (e.g.
# `CUDA_VISIBLE_DEVICES=1 ./scripts/run_pipeline_conformer_kernel3_6blocks.sh`),
