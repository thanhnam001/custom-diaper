...
# Runs the 3-stage SC_LibriSpeech_2spk_conformer_kernel31 recipe end-to-end
...
#
# Configs: models/10attractors/SC_LibriSpeech_2spk_conformer_kernel31/*.yaml
...
# etc.) with exactly one change: conformer_conv_kernel_size: 31 instead of 3
# (the kernel size used in the original Conformer paper; conv_norm_type
# stays batchnorm, same as that baseline -- see
# models/10attractors/SC_LibriSpeech_2spk_conformer_kernel5 for the
# kernel-5 variant and models/10attractors/SC_LibriSpeech_2spk_conformer_layernorm
# for the LayerNorm variant). Everything else -- including
# latents2attractors: weighted_average -- matches that baseline, so the two
# recipes differ in only that one variable.
#
...
CONFIG_DIR="models/10attractors/SC_LibriSpeech_2spk_conformer_kernel31"
# Respects an already-set CUDA_VISIBLE_DEVICES from the calling shell (e.g.
# `CUDA_VISIBLE_DEVICES=1 ./scripts/run_pipeline_conformer_kernel31.sh`),
