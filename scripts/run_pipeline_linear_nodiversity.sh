...
# Runs the 3-stage SC_LibriSpeech_2spk_linear_nodiversity recipe end-to-end on
...
#
# Configs: models/10attractors/SC_LibriSpeech_2spk_linear_nodiversity/*.yaml
# Difference from SC_LibriSpeech_2spk_linear_diversity: attractor_diversity_loss_weight
# is omitted (defaults to 0.0, which also skips computing the term at all --
# see diaper/train.py::compute_loss_and_metrics), so training runs on 2 active
# loss terms only (activation_loss_BCE + attractor_existence_loss) instead of 3.
# Everything else (latents2attractors: linear, auto-computed noam schedule)
# is unchanged from that recipe.
#
...
CONFIG_DIR="models/10attractors/SC_LibriSpeech_2spk_linear_nodiversity"
# Respects an already-set CUDA_VISIBLE_DEVICES from the calling shell (e.g.
# `CUDA_VISIBLE_DEVICES=1 ./scripts/run_pipeline_linear_nodiversity.sh`),
