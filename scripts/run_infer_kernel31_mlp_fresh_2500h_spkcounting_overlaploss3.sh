...
# SC_LibriSpeech_2spk_conformer_kernel31_mlp_fresh_2500h_spkcounting_overlaploss3
# lineage
# (models/10attractors/SC_LibriSpeech_2spk_conformer_kernel31_mlp_fresh_2500h_spkcounting_overlaploss3/):
...
# (overlap_loss_weight 3.0) lineage instead.
#
# --overlap-loss-weight is train-only (diaper/backend/losses.py::
# pit_loss_multispk); it doesn't change the model architecture, so nothing
# else about inference/scoring here differs from the sibling script.
#
# To directly compare against exp13's mlp_unmaskeddiv baseline's per-head
# breakdown (not just DER) -- OSD_FA/miss=40.26/69.35 at epochs156-166 on
# MSDWild -- use infer.py --compute-metrics --collar 0.25 or
# diaper/oracle_attractor_analysis.py instead of/alongside this script.
#
# Run from the repo root, on the server:
#   ./scripts/run_infer_kernel31_mlp_fresh_2500h_spkcounting_overlaploss3.sh
...
RUNS=(
    "msdwild_mlp_spkcounting_overlaploss3|${CONFIG_DIR}/infer_msdwild_mlp.yaml|0.25"
    "ramc_mlp_spkcounting_overlaploss3|${CONFIG_DIR}/infer_ramc_mlp.yaml|"
)
