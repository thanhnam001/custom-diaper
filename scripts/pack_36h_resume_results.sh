#!/bin/bash
# Pack everything scripts/run_36h_resume_queue.sh produced, to bring back
# from the server. Run ON THE SERVER, from the repo root:
#
#   ./scripts/pack_36h_resume_results.sh                 # ~0.5 GB, no weights
#   DRY_RUN=1 ./scripts/pack_36h_resume_results.sh        # just size it
#   MODE=avg ./scripts/pack_36h_resume_results.sh         # + last $KEEP checkpoints
#
...
# Same semantics as pack_results_from_server.sh -- see its header for detail.
#
# A stage marked NOT RUN never started (its dependency was missing, or the
# lane aborted preflight). A stage with checkpoints but no DER line trained
# but was not scored yet -- most likely its RAMC scoring was still queued
# behind the cross-lane lock when the queue's 36h ended; its weights are in
# the archive under MODE=avg/all, so score it locally afterward.

set -eu

MODE="${MODE:-results}"
OUT="${OUT:-diaper_36h_resume_results.tar}"
KEEP="${KEEP:-10}"
COMPRESS="${COMPRESS:-0}"
DRY_RUN="${DRY_RUN:-0}"
ONLY_LANE="${ONLY_LANE:-}"
...
BYTES=0; NFILES=0; MISSING=0; ADDED_N=0; ADDED_B=0
...
_probe=$(yaml_get output_path "$CNF_DIR/finetune_ramc_10spks.yaml")
if [ -z "$_probe" ]; then
    echo "ERROR: could not read output_path from $CNF_DIR/finetune_ramc_10spks.yaml." >&2
    echo "       Run from the repo root, or set CNF_DIR." >&2
    exit 1
fi
# output_path is .../experiments/10attractors/<run>/models_finetuneRAMC ->
...
say "DiaPer 36h resume queue -- results pack"
...
if [ -z "$ONLY_LANE" ] || [ "$ONLY_LANE" = "A" ]; then
    say "LANE A (conformer_k31)"
    ramc=$(yaml_get output_path "$CNF_DIR/finetune_ramc_10spks.yaml")
    msd=$(yaml_get output_path "$CNF_DIR/finetune_msdwild_10spks.yaml")
    pack_stage A "A2 RAMC (base)"        "$ramc"
    pack_stage A "A5 RAMC @ subsampling5" "${ramc}_sub5"
    pack_stage A "A1 MSDWild (base)"     "$msd"
    pack_stage A "A3 RAMC @ lr1e-5"      "${ramc}_lr1e-5"
    pack_stage A "A4 RAMC @ lr3e-6"      "${ramc}_lr3e-6"
fi

if [ -z "$ONLY_LANE" ] || [ "$ONLY_LANE" = "B" ]; then
    say ""
    say "LANE B (ebf / paperlr)"
    ple_ramc=$(yaml_get output_path "$PLE_DIR/finetune_ramc_10spks.yaml")
    ebf_ramc=$(yaml_get output_path "$EBF_DIR/finetune_ramc_10spks.yaml")
    plr_msd=$(yaml_get output_path "$PLR_DIR/finetune_msdwild_10spks.yaml")
    pack_stage B "B4 paperlr_ebf RAMC (base)"          "$ple_ramc"
    pack_stage B "B2 fixednoam_ebf RAMC @ subsampling5" "${ebf_ramc}_sub5"
    pack_stage B "B3 paperlr vanilla MSDWild (base)"    "$plr_msd"
    pack_stage B "B1 fixednoam_ebf RAMC @ lr1e-5"       "${ebf_ramc}_lr1e-5"
fi
...
ADDED_N=0; ADDED_B=0
stage_repo_file scripts/run_36h_resume_queue.sh "run_36h_resume_queue.sh"
say "  driver   $ADDED_N script(s) (the per-stage timeout/patience overrides live here)"
say "  $(human $(( BYTES - b0 ))) added"
...
say ""
say "Extract locally: mkdir -p ~/diaper_36h && tar -xf $(basename "$OUT") -C ~/diaper_36h"
