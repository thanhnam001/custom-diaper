...
# SC_LibriSpeech_2spk_conformer_kernel31_mlp_fresh_2500h_spkcounting
# lineage
...
#
# Same as scripts/run_infer_kernel31_mlp_fresh_2500h.sh (see that script's
# header for the full rationale: server-local absolute paths used as-is,
# epochs auto-detected from whatever checkpoints exist and averaged over
# the last $MAX_CHECKPOINTS_TO_AVERAGE, a config skipped if its models_path
# has no checkpoints yet, reference RTTM derived from infer_data_dir) --
# only CONFIG_DIR changes to point at this (spk-counting) lineage instead.
#
# Run from the repo root, on the server:
#   ./scripts/run_infer_kernel31_mlp_fresh_2500h_spkcounting.sh
...
RUNS=(
    "msdwild_mlp_spkcounting|${CONFIG_DIR}/infer_msdwild_mlp.yaml|0.25"
    "ramc_mlp_spkcounting|${CONFIG_DIR}/infer_ramc_mlp.yaml|"
)
